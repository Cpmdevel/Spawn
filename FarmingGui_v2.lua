-- ============================================================
--  🌱 FARMING GAME - FarmingGui.lua  v2.0
--  ✅ Executor compatible (getgenv, pcall-safe remotes)
--  ✅ Mobile / PC input support
--  ✅ Draggable panel
--  ✅ Harvest All button
--  ✅ Sell per-item button
--  ✅ Seed stock counter in shop
--  ✅ Auto-refresh timer display
--  ✅ Minimise / Close toggle
--  Place inside: StarterGui (LocalScript inside ScreenGui)
--        OR as LocalScript in StarterPlayerScripts
-- ============================================================
...
until Remotes or waited >= 15

if not Remotes then
    warn("[FarmingGui] Remotes folder not found after 15s – aborting.")
    return
end
...
local function safeWait(parent, name, timeout)
    ...
    local start = tick()
    repeat task.wait(0.05) obj = parent:FindFirstChild(name)
    until obj or (tick() - start) >= timeout
    return obj
end

local RE_PlantSeed   = safeWait(Remotes, "PlantSeed")
local RE_HarvestPlot = safeWait(Remotes, "HarvestPlot")
local RE_HarvestAll  = safeWait(Remotes, "HarvestAll")   -- new (optional)
local RE_SellAll     = safeWait(Remotes, "SellAll")
local RE_SellOne     = safeWait(Remotes, "SellOne")      -- new (optional)
local RE_BuySeed     = safeWait(Remotes, "BuySeed")
local RE_UpdateUI    = safeWait(Remotes, "UpdateUI")
local RF_GetSeeds    = safeWait(Remotes, "GetSeeds")
...
do
    ...
    -- TopBar is defined below; we attach drag to it after creation
    ...
    -- Store for use after topBar is built
...
local topBar = makeFrame(container, "TopBar",
    ...
    UDim2.new(0, 0, 0, 0),
    C.topbar, 0)
-- Square top corners only
local tcorner = Instance.new("UICorner")
tcorner.CornerRadius = UDim.new(0, 14)
tcorner.Parent = topBar

-- Title
local titleLabel = makeLabel(topBar, "🌱 FARMING GAME  v2",
    UDim2.new(1, -130, 1, 0),
    UDim2.new(0, 14, 0, 0),
    C.green, 17, Enum.TextXAlignment.Left)

-- Coin display (right side)
local coinLabel = makeLabel(topBar, "🪙 0",
    UDim2.new(0, 110, 1, 0),
    UDim2.new(1, -118, 0, 0),
    C.gold, 15, Enum.TextXAlignment.Right)
...
makeLabel(seedsPanel, "🛒 Seed Shop",
    ...
    C.green, 15, Enum.TextXAlignment.Center)
...
local qtyPlus = makeButton(seedsPanel, "+",
    UDim2.new(0, 32, 0, 32),
    UDim2.new(0, 116, 1, -38),
    C.green, C.white, 6)
local buyBtn = makeButton(seedsPanel, "BUY SEED",
    UDim2.new(0, 145, 0, 32),
    UDim2.new(1, -153, 1, -38),
    C.gold, C.bg, 8)
buyBtn.Name    = "BuyBtn"
buyBtn.TextSize = 14
...
makeLabel(gardenPanel, "Seed to plant:",
    UDim2.new(0.55, -10, 0, 20),
    UDim2.new(0, 8, 0, 268),
    C.dim, 12)

local plantSeedScroll = Instance.new("ScrollingFrame")
plantSeedScroll.Name               = "PlantSeedScroll"
plantSeedScroll.Size               = UDim2.new(0.55, -8, 0, 34)
plantSeedScroll.Position           = UDim2.new(0, 8, 0, 290)
...
local totalValueLabel = makeLabel(sellPanel, "Total Value: 🪙0",
    UDim2.new(1, -20, 0, 22),
    UDim2.new(0, 10, 1, -72),
    C.gold, 14, Enum.TextXAlignment.Center)
...
local notifFrame = makeFrame(screenGui, "Notif",
    UDim2.new(0, 270, 0, 46),
    UDim2.new(0.5, -135, 1, 10),  -- starts offscreen below
    Color3.fromRGB(28, 80, 28), 12)
notifFrame.ZIndex = 10
stroke(notifFrame, Color3.fromRGB(55, 130, 45), 1)

local notifLabel = makeLabel(notifFrame, "",
    UDim2.new(1, -12, 1, 0),
    UDim2.new(0, 6, 0, 0),
    C.white, 13, Enum.TextXAlignment.Center)
notifLabel.ZIndex = 11

local notifThread
local function showNotif(text, color)
    notifFrame.BackgroundColor3 = color or Color3.fromRGB(28, 80, 28)
    notifLabel.Text = text
    notifFrame.Visible = true
    notifFrame.Position = UDim2.new(0.5, -135, 1, 10)
    TweenService:Create(notifFrame, TweenInfo.new(0.25, Enum.EasingStyle.Back), {
        Position = UDim2.new(0.5, -135, 1, -62)
    }):Play()
    if notifThread then task.cancel(notifThread) end
    notifThread = task.delay(2.2, function()
        TweenService:Create(notifFrame, TweenInfo.new(0.2), {
            Position = UDim2.new(0.5, -135, 1, 10)
        }):Play()
        task.delay(0.25, function() notifFrame.Visible = false end)
    end)
end
...
local function buildSeedShop(seeds)
    ...
    for _, seedName in ipairs(names) do
        ...
        -- Grow / sell info
        makeLabel(card, "⏱ " .. info.growTime .. "s  →  🪙" .. info.sellPrice,
...
local function updateSellUI(inventory, seeds)
    ...
    if not hasItems then
        local empty = makeLabel(inventoryScroll, "No crops to sell.\nGrow some first! 🌱",
            UDim2.new(1, -10, 0, 60),
            UDim2.new(0, 5, 0, 10),
            C.dim, 13, Enum.TextXAlignment.Center)
    end
...
-- ============================================================
--  PLANT SEED BUTTONS (Garden tab bottom)
-- ============================================================
local plantSeedBtns = {}
...
plantBtn.MouseButton1Click:Connect(function()
    ...
    plotButtons[selectedPlot].BackgroundColor3 = C.plotEmpty
    selectedPlot = nil
end)
...
harvestAllBtn.MouseButton1Click:Connect(function()
    ...
    if RE_HarvestAll then
        ...
        -- Fallback: fire individual harvests
        for i, plot in ipairs(playerPlots) do
            if plot.state == "ready" then
                RE_HarvestPlot:FireServer(i)
            end
...
RE_UpdateUI.OnClientEvent:Connect(function(data)
    ...
    -- Coin display (1K format)
    local display = playerCoins >= 1000
        and string.format("%.1fK", playerCoins / 1000)
        or  tostring(playerCoins)
    coinLabel.Text = "🪙 " .. display

    -- Rebuild seed shop only once
    if #seedCards == 0 then
        buildSeedShop(seedCatalog)
    else
        -- Update stock labels only
        for _, sc in ipairs(seedCards) do
            local inv = playerInventory[sc.name] or 0
            if sc.stockLabel then
                sc.stockLabel.Text = "In bag: " .. inv
            end
        end
    end
...
-- ============================================================
--  GARDEN TIMER LOOP  (1 s local countdown)
-- ============================================================
task.spawn(function()
